...
if __name__ == "__main__":
    ...
    for i in range(len(z)):
        ...
        print("agent {} average bid is {}".format(i+1, roll_m.iloc[-1]))
    ...
    for i in range(len(z)):
        ...
        df_down = pd.DataFrame(agents_b_down[i])
        roll_m = df_m.rolling(100).mean()
        print("agent {} delivered quality is {}".format(i+1, roll_m.iloc[-1]))
    ...
    for i in range(len(z)):
        ...
        df_down = pd.DataFrame(agents_b_down[i])
        roll_m = df_m.rolling(100).mean()
        print("agent {} effort level is {}".format(i+1, roll_m.iloc[-1]))
