...
agents = [DDPGAgent(3, [512, 256, 128], 2, actor_learning_rate = 5.e-5, 
          critic_learning_rate = 5.e-4, max_memory_size = 80_000) for _ in range(n_agents)]

principal = DQNAgent(n_agents*2, [256, 256, 128], n_agents+1, learning_rate = 0.00005, 
                     min_eps = 0.0, epsilon_decay = 0.999, max_memory_size = 80_000)

winner = DQNAgent(3, [512, 512, 256], 40, learning_rate = 0.0001, \
                min_eps = 0.0, epsilon_decay = 0.999, max_memory_size = 80_000)
...
noise = OUNoise(env.action_space, mu=.0, theta=0., max_sigma=.4, min_sigma=.4)
...
principal_action = principal.get_action(torch.FloatTensor(principal_state), 1, 0)[0]
...
winner.action = winner.get_action(winner.state, 3.0, 0)
...
for episode in range(n_episodes):
    ...
    for step in range(steps_per_episode):
        ...
        winner.action = winner.get_action(winner.new_state, 3.0, episode)
        ...
        if q + epsilon < agents[selectee].action[1]:
            agents[selectee].reward += -1.0 + (q - agents[selectee].action[1])
            principal_reward += -3.0
            winner.reward += -1.0 + (q - agents[selectee].action[1])
            success = 0
            done = True
        else:
            winner.reward += (1.0*agents[selectee].action[0] - cost[selectee] * (max_try+1))
